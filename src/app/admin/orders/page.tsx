...
import { STATUS_LABELS, PAYMENT_LABELS, ORDER_STATUSES, type OrderStatus } from "@/lib/orders";
          ...
          <input
            ...
            onKeyDown={async (e) => {
              if (e.key === "Enter") {
                ...
                window.location.href = `/admin/orders${v ? `?search=${encodeURIComponent(v)}` : ""}`;
              }
            }}
          />
          <select
            ...
            onChange={(e) => {
              ...
              window.location.href = `/admin/orders${s ? `?status=${s}` : ""}`;
            }}
